...
var PewpewVaariables = map[string]*VariableVal{{
  // Enums
  {enum_descriptions}
...
def generate_api(pewpew_lib: dict) -> str:
    ...
    return _PEWPEW_API_TEMPLATE.format_map(
        {
            "enums": "\n".join(enum.generate_str() for enum in enums),
            "enum_descriptions": "\n\t".join(enum.generate_desc() for enum in enums),
            "functions": "\n".join(function.generate_str() for function in functions),
        }
    )
...
def _generate_enum_docs(enum: types.APIEnum) -> str:
    ...
    enum_template += "".join(
        [
            f"\n- `{mappings.get(value, helpers.to_pascal_case)}`"
...
def _generate_function_docs(function: types.APIFunction) -> str:
    processed_name = mappings.get(function.name, helpers.to_pascal_case)
    return_types = (
        (
            "-> "
            + ", ".join(
    ...
    function_template += f"{helpers.to_camel_case_all(function.description)}"
